...
class UserController extends AbstractActionController
{
    ...
    const ROUTE_CHANGEDISPLAYNAME  = 'zfcuser/changedisplayname';
    ...
    protected $changeDisplayNameForm;
    ...
    public function changedisplaynameAction()
    {
    	...
    	$form = $this->getChangeDisplayNameForm();
    	$prg = $this->prg(static::ROUTE_CHANGEDISPLAYNAME);
    
    
    	$fm = $this->flashMessenger()->setNamespace('change-displayname')->getMessages();
    	if (isset($fm[0])) {
    		$status = $fm[0];
    	} else {
    		$status = null;
    	...
    	if (!$this->getUserService()->changeDisplayName($form->getData())) {
    		return array(
    				'status' => false,
    				'changeDisplayNameForm' => $form,
    		);
    	...
    	$this->flashMessenger()->setNamespace('change-displayname')->addMessage(true);
    	return $this->redirect()->toRoute(static::ROUTE_CHANGEDISPLAYNAME);
    }
    ...
    public function changeEmailAction()
    {
        ...
        $request = $this->getRequest();
        $request->getPost()->set('identity', $this->getUserService()->getAuthService()->getIdentity()->getId());
    ...
    public function getChangeDisplayNameForm()
    {
    	if (!$this->changeDisplayNameForm) {
    		$this->setChangeDisplayNameForm($this->getServiceLocator()->get('zfcuser_change_displayname_form'));
    	}
    	return $this->changeDisplayNameForm;
    }

    public function setChangeDisplayNameForm(Form $changeDisplayNameForm)
    {
    	$this->changeDisplayNameForm = $changeDisplayNameForm;
    	return $this;
    }
